...
func (oq *OrderQuery) QueryDelivery() *DeliveryQuery {
	query := &DeliveryQuery{config: oq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// the "orderItems" edge. The optional arguments used to configure the query builder of the edge.
...
//  WithDelivery tells the query-builder to eager-loads the nodes that are connected to
// the "delivery" edge. The optional arguments used to configure the query builder of the edge.
func (oq *OrderQuery) WithDelivery(opts ...func(*DeliveryQuery)) *OrderQuery {
	query := &DeliveryQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
	oq.withDelivery = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context) ([]*Order, error) {
	...
	if query := oq.withDelivery; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Order)
		for i := range nodes {
			if fk := nodes[i].delivery_id; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "delivery_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Delivery = n
			}
		}
	}

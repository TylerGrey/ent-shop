...
// The DeliveryQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type DeliveryQueryRuleFunc func(context.Context, *ent.DeliveryQuery) error

// EvalQuery return f(ctx, q).
func (f DeliveryQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.DeliveryQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.DeliveryQuery", q)
}

// The DeliveryMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type DeliveryMutationRuleFunc func(context.Context, *ent.DeliveryMutation) error

// EvalMutation calls f(ctx, m).
func (f DeliveryMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.DeliveryMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.DeliveryMutation", m)
}
